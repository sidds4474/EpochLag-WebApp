...
export default function GroupDetailPage({
  ...
  async function handleAdd(list: PickerUser[]) {
    ...
    try {
      ...
      setGroup({
        ...group,
        members: merged,
        memberCount: (group.memberCount ?? group.members?.length ?? 0) + list.length,
      });
  ...
  async function handleLeave() {
    ...
    try {
      ...
      router.push("/friends-and-family");
    } catch (e) {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 bg-white">
      <div className="px-[16px] pt-[16px] flex items-center gap-[12px]">
        <button
          type="button"
          onClick={() => router.back()}
          aria-label="Back"
        ...
        <h1 className="font-montserrat font-bold text-primary-blue text-[22px]">
          {group?.name ?? "Group"}
        </h1>
      </div>

      <div className="flex-1 overflow-y-auto px-[16px] pt-[16px] pb-[16px]">
        {loading ? (
          <div className="flex flex-col gap-[10px]">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-[#f3f3f3] animate-pulse rounded-[10px] h-[56px]" />
            ))}
          </div>
        ) : !group ? (
          ...
          <div className="flex flex-col">
            {(group.members ?? []).map((m) => (
              ...
              >
                ...
                <ChevronRightIcon width={16} height={16} className="text-primary-blue/50" />
              </button>
            ))}
          </div>
        )}
      ...
      <div className="px-[16px] pb-[24px] pt-[8px] flex flex-col items-center gap-[10px]">
        <button
          type="button"
...
function Avatar({ member }: { member: PersonLike }) {
  const url = bustUrl(member.profilePicture, member.updatedAt);
  return (
    <div className="w-[44px] h-[44px] rounded-full overflow-hidden bg-primary-blue/15 text-primary-blue flex items-center justify-center font-montserrat font-semibold text-[14px] shrink-0">
      {url ? (
...
function LeaveIcon({ width = 20, height = 20 }: { width?: number; height?: number }) {
  return (
